...
bool invoke_driver(fptr_data::kernel_com* com, fptr_data::kernel_opr op)
{
	...
	if (!pHookFunc((uintptr_t)com, 0, 0xBADC0DE, (int32_t)op, 2) &&
		com->error == fptr_data::kernel_err::unset_err)
	{
...
bool kernel::driver::init()
{
	if (!pHookFunc)
	{
		...
		pHookFunc = (pfunc_hk_t)GetProcAddress(hDll, ("NtGdiPolyPolyDraw"));
		if (!pHookFunc)
		{
...
uintptr_t kernel::driver::move(long x, long y, unsigned short button)
{
	fptr_data::kernel_com com{};
	com.x = x;

	com.y = y;
	com.button_flags = button;

	invoke_driver<true>(&com, fptr_data::kernel_opr::move_mouse);
