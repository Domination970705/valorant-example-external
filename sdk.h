...
namespace SDK
{
	...
	inline void renderBoneLine(Vector3 first_bone_position, Vector3 second_bone_position) {
		...
		ImGui::GetBackgroundDrawList()->AddLine(fist_screen_position, second_screen_position, FtIM2(bone_color), 2);
	}
	...
	inline void renderBones(EnemyPtr enemy) {
		...
		if (enemy.bone_count == 102) { // MALE & FEMALE
			...
			neck_position = GetBone(enemy.mesh_ptr, 21);

			l_upper_arm_position = GetBone(enemy.mesh_ptr, 45);
			l_fore_arm_position = GetBone(enemy.mesh_ptr, 42);
			l_hand_position = GetBone(enemy.mesh_ptr, 44);

			r_upper_arm_position = GetBone(enemy.mesh_ptr, 48);
			r_fore_arm_position = GetBone(enemy.mesh_ptr, 67);
			r_hand_position = GetBone(enemy.mesh_ptr, 69);
			...
			r_thigh_position = GetBone(enemy.mesh_ptr, 82);
			r_knee_position = GetBone(enemy.mesh_ptr, 84);
			r_foot_position = GetBone(enemy.mesh_ptr, 85);
...
#define Const_RadToUnrRot                          10430.3783504704527
	...
	int ActorLoop()
	{
		...
		for (int i = 0; i < local_enemy_collection.size(); i++)
		{
			...
			//FVector root_position = read<FVector>(enemy.root_component_ptr + ROOT_POS);
				if (!IsVisible(enemy.mesh_ptr) || !vischeck)
				{
					continue;
				}
	...
	inline void renderBox(Vector3 head_at_screen, float distance_modifier) {
		...
		ImGui::GetBackgroundDrawList()->AddRect(ImVec2(start_x, start_y), ImVec2(end_x, end_y), FtIM2(box_color));
	}
	static Vector3 pRadar;
	ImVec4 R1 = to_vec4(255, 0, 0, 255);
	ImVec4 S4 = to_vec4(0, 255, 255, 255);
	...
	void DrawRadar(Vector3 EntityPos)
	{
		...
		auto Radar2D = WorldRadar(LocalPos, EntityPos, camera.Rotation.y, radar_posX, radar_posY, 100.f);
		DrawCircleRadar(Radar2D.x, Radar2D.y, 3, R1);
	}
}